...
The original effect size of the main test (chi-square between those who cheat v. not in the self alone as compared to the self-and-other) was .14. A power analysis revealed that we would need a sample of 401 particiapnts to detect such an effect with 80% power. Given bugdet constraints of having $100 to run the study, and factoring each participant would get paid 40 cents base pay + some variable pay, we intend to collect 80 participant data and, based on how much funds we have left over, will run more subjects until we run out of $100 (it is hard to know a priori the exact number because people will get paid based on how many words they report unscrambling.)

###Planned Sample

Our planned sample size is as much as we can collect with the $100 budget. We will stop data collection after we run out of funds. Data collection will occur on Amazon Mechanical Turk, limited to US residents only. 
...
cheatfrequency <- d1 %>% group_by(Condition, cheat_yes) %>% dplyr::summarise(freq=n())
...
counts <- as.table(matrix(c(percent_cheat_self, percent_cheat_other, percent_cheat_self_original, percent_cheat_other_original), byrow=T, ncol=2))
colnames(counts) <- c("Original", "Replication")
rownames(counts) <- c("Self & Other", "Self Alone")

barplot(counts, main="Percent Cheat in Self vs. Other: Original Compared to Replication", 
  	xlab="Original v. Replication", col=c("darkblue","red"),
